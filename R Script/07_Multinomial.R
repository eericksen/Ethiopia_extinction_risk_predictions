...
# Add CIs and sort by x within each group
long_df_ci <- long_df %>%
  mutate(
    ci_lower = case_when(
...
# Define your colours & labels once
my_cols <- c(
...
eff_long_fr <- as.data.frame(Effect("hfp_delta_class5", model_01, xlevels = list(hfp_delta_class5 = seq(0,1.0, by = 0.05)))) %>% 
  ...
  # make sure "not.threatened" → "not_threatened"
  mutate(
    HFP5 = gsub("\\.", "_", HFP5),
    HFP5 = factor(
      HFP5,
...
ggplot(eff_long_fr, aes(hfp_delta_class5, Probability, fill = HFP5)) +
  # default stacking (bottom → top) now follows your factor levels
  geom_area(position = "stack") +
  scale_fill_manual(
    name   = "Prediction",
    breaks = c("under", "threatened", "not_threatened", "over"),
  ...
  theme(
    legend.position     = "bottom",
    legend.box          = "vertical",   # stack title & keys if title.position="bottom"
    legend.title.align  = 0.5           # center the title under the keys
  )
...
ggplot(eff_long_fr,
  ...
  scale_fill_manual(
    ...
    breaks = c("under", "threatened", "not_threatened", "over"),
    labels = c(
      "Under-predicted",         # ASCII hyphen here
      "Correct (threatened)",
      "Correct (not threatened)",
      "Over-predicted"           # ASCII hyphen here
    ),
    values = pal
  ) +
  
  # line (colour) scale – same name so it re‑uses the title
  scale_colour_manual(
    name   = "Prediction",
    breaks = c("under", "threatened", "not_threatened", "over"),
    labels = c(
